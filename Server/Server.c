...
int main(int argc, char * argv[])
{
	...
	for (int i = 0; i < MAX_CLIENTS; i++) {
		...
		Players[i].valid = 0;
		Players[i].turn_finished = NewEvent(NULL, AUTO_RESET);
		Players[i].type_choosed = NewEvent(NULL, AUTO_RESET);
	}
	...
	////////////////////////////////////// Disconnect(both player if they didn't did it already)
...
int NewUser(Player *player, char *username) {
	...
	////////////////////////////// update TotalPlayersHistory number - mybe we should move it to function when game finish??
...
int UpdateScoreBoard(Player *player, int is_win) {
	...
	errno_t err;
	err = fopen_s(&leader_board_file_p, GAME_SESSION_FILE_NAME, "w");
	if (err != 0) {
		printf("Unable to create file.\n");
		ReleaseMutex(TotalPlayersHistoryMutex);
...
int PlayVersus(Player *player, Player *other_player, BOOL *create_game_session, BOOL replay) {
	...
	if (other_player->play_type != VERSUS) {
		if (!replay) {			// send SERVER_NO_OPPONENTS at firs round otherwise send SERVER_OPPONENT_QUIT 
			if (SendString(SERVER_NO_OPPONENTS, player->player_socket) == TRNS_FAILED)
			{
				printf("Socket error while trying to write data to socket\n");
				return STATUS_CODE_FAILURE;
...
static DWORD WaitForConnection(LPVOID lpParam) {
	...
	{
		/*
		Timeout.tv_sec = TimeToWait - (long)time(NULL); //Update timeout
		if (Timeout.tv_sec <= 0) {
			break;
		}
		ReadySockets = select(FD_SETSIZE, &SocketsToCheck, NULL, NULL, &Timeout);
		if (ReadySockets != 1) {//Check for normal activity of select function
			break;
		}
		*/
